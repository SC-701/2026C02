...
namespace DA
{
    public class UsuarioDA : IUsuarioDA
    {
        ...
        public async Task<IEnumerable<Perfil>> ObtenerPerfilesxUsuario(Guid idUsuario)
        {
            string sql = @"[ObtenerPerfilesxUsuario]";
            var resultado = await _sqlConnection.QueryAsync<Perfil>(sql, new { IdUsuario = idUsuario });
            return resultado;
        }
