...
namespace Autorizacion.Middleware
{
    public class ClaimsPerfiles
    {
        ...
        {
            var idUsuarioTexto = httpContext.User.Claims
                .FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value;

            if (!Guid.TryParse(idUsuarioTexto, out var idUsuario))
            {
                return Enumerable.Empty<Perfil>();
            }

            return await _autorizacionFlujo.ObtenerPerfilesxUsuario(new Abstracciones.Modelos.Usuario
            {
                Id = idUsuario
            });
        }
