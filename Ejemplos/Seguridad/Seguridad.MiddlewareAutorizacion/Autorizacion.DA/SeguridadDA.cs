...
namespace Autorizacion.DA
{
    public class SeguridadDA : ISeguridadDA
    {
        ...
        public async Task<IEnumerable<Perfil>> ObtenerPerfilesxUsuario(Usuario usuario)
        {
            ...
            var resultado = await _sqlConnection.QueryAsync<Perfil>(sql, new { IdUsuario = usuario.Id, CorreoElectronico = usuario.CorreoElectronico, NombreUsuario = usuario.NombreUsuario });
            return resultado;
        }
